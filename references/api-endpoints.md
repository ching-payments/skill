...
**Embedded checkout.** Every one-time price or cart session is also embeddable as a button-only widget via the `embed_url` (`/checkout/:id/embed`) - render it in an `<iframe allow="payment">` to keep the customer on your page instead of redirecting. `payment_methods` picks which buttons appear and `background_color` themes it (text colors are derived from the background). `auto_open_wallets` opens the digital-wallet sheet automatically on load (a loader shows first, and dismissing it falls back to the buttons); it requires wallets enabled for the account and `"wallet"` among `payment_methods`. The iframe needs `allow="payment"` + HTTPS for Apple Pay / Google Pay (card works regardless). On completion the embed posts `{ type: "ching-checkout-result", status: "success" | "failed", session_id }` to the parent window - a UX signal only; fulfil on the `charge.succeeded` webhook. Embedded checkout is immediate-charge only (one-time price or cart); `payment_methods` is rejected for recurring/mixed.
...
| POST | `/subscriptions/:id/change_price` | Switch the subscription to a different recurring price without canceling. Required body: `price` (recurring price visibleId; a one-time price is rejected with `price_not_recurring`). Optional: `change_timing` (`immediately` \| `at_period_end`). Auto-routes by default: an **upgrade** (new price costs more per day) applies now and is charged **prorated for the days left** in the current period (`current_period_end` is unchanged - the full price bills at the unchanged renewal); a **downgrade** is scheduled to apply at `current_period_end`. The current period is never refunded, so `change_timing: immediately` on a downgrade returns `400 DOWNGRADE_IMMEDIATE_NOT_SUPPORTED`; `at_period_end` defers an upgrade to renewal with no charge now. Returns `{ kind: upgrade\|downgrade\|same, applied, scheduled_for, charge_amount, charge_currency, subscription }` - `applied` true when the new price is live now (else scheduled), `charge_amount`/`charge_currency` describe the prorated charge taken now (null when nothing charged). An applied upgrade swaps the item and issues a tax document (`charge.succeeded`); either path fires `subscription.updated`. Switching to the current price is a no-op (`kind: same`) that clears any pending change. |
